...
**Who does what — Claude diagnoses, Lukas codes and verifies.** The setup guide exists so **Lukas writes the code** — copying and typing code into the right places is how he stays on top of the app, and it stands in for not hand-authoring from scratch. So the guide carries the **full code to type**, not just prose. **Claude's job is the diagnostics** — running the app, probes, and experiments to work out _what_ needs building — and authoring the guide. The default is: Claude writes the guide → Lukas codes from it → **Lukas does all the testing and verification** (he works through the app's difficulties himself; he lists lint errors / bugs back to Claude in a follow-up). **Claude does NOT run the test suite to "prove" a coded change** — it runs only the diagnostics needed to decide what to build. The one exception: in a **volatile phase** (fast iteration, strategy still moving) Claude may do the heavy coding out of necessity — but even then testing + verification stay with Lukas. When in doubt, Claude writes the guide and stops; Lukas drives the keyboard.
