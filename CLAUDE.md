...
- **Phase 3e — CV JSON export/import (2026-06-10).** New `cv_export` management command dumps a user's whole CV to the by-name JSON shape `cv_import` consumes (now incl. `related_skills`, `years_of_experience_override`, `ResumeSnippet`); `cv_import` extended to round-trip those and to scope domains/locations to the target user (fixing a latent non-null-FK crash) with auto-create-on-miss so a fresh server need not pre-share the taxonomy. Round-trip test pins parity. For migrating data onto a deployed box. Live setup guide at [.claude/plans/phase-3e-setup-guide.md](.claude/plans/phase-3e-setup-guide.md).
- **Phase 3a-bis — career-model relation completion (2026-06-10).** Closed the unmodeled cross-entry relations before bulk CV entry: directed `Skill.builds_on` (`symmetrical=False`, reverse `enables`; distinct from symmetric `related_skills`), `Education.skills`/`domains`, `Certification.skills` (reverse `certifications`)/`domains`, `Project.job` FK, `ResumeSnippet.job`/`project` FKs. All scoped + writable in DRF (`builds_on` self-reject + read-only `enables`), round-tripping through `cv_export`/`cv_import` (cert.skills wired in a deferred pass since certs import before skills; source jobs referenced by title). New `JobPicker` (single-FK) + `builds_on`/skills/domains/job wired into the skills/education/certifications/projects editors. Pipeline *consumption* of these edges deferred to 3f. 113 backend tests green. Live setup guide at [.claude/plans/phase-3a-bis-setup-guide.md](.claude/plans/phase-3a-bis-setup-guide.md).
- **Phase 3d-bis — CV list UX (2026-06-12, uncommitted in working tree).** Server-side `Pagination` for every `/cv/*` section list (DRF `PAGE_SIZE` 50→100; `usePagedList` owns page state and snaps back to page 1 when search/filters change so a narrowed filter can't 404 on an out-of-range page; `useFullList`/`fetchAll` in [queries/jac.ts](frontend/src/lib/queries/jac.ts) + [queries/paginated.ts](frontend/src/lib/queries/paginated.ts) walk all pages for views that can't page). Single-select `DomainFilter` combobox wired into all section toolbars (`filterset_fields=["domains"]` added to the Education + Certification viewsets). New `SkillBubbles` packed-circle cloud ([components/cv/skill-bubbles.tsx](frontend/src/components/cv/skill-bubbles.tsx), d3-hierarchy: bubble area = proficiency, color = category, click = open editor) behind a table/bubbles toggle on `/cv/skills`. `SkillPicker` gained opt-in `autoAddPrerequisites` — adding a skill pulls in its transitive `builds_on` closure (add-only, cycle-guarded, toast-announced) on the jobs/projects/education/certs editors. New deps: `d3-hierarchy` + `@types/d3-hierarchy`. 206 backend tests green; `tsc -b` clean. No setup guide authored yet (gap).

**Next, in order** (full detail in the roadmap):

- **Phase 2d — LLM connector UI (immediate next — unblocks CV-tool dogfooding).** `/settings/llm` (CRUD over `LLMConfig` with write-only `api_key`) + `/settings/llm/usage` (read-only `LLMRequestLog` with date filter + aggregate spend). Pulled to the front because adding a real (paid) LLM key via the UI is the precondition for the 3f JAC-core validation gate — the CV pipeline can't be dogfooded against the free Ollama default alone.
- **Phase 3 (remaining) — JAC-core gate → hardening/SPA/localization.** Re-prioritised 2026-06-10; 3c/3d (editor completeness) + 3e (export/import) shipped. (3f) JAC-core validation gate — dogfood `cv_test`/`ai_tailor_with_fallback` over real postings, log findings, Phase 4 go/no-go; then (3g) pre-deployment hardening — throttles, N+1 audit, security headers/secure cookies, validation, multipart/avatar, wider reauth; (3h) first SPA backend (`PortfolioLink` + `VisitorResponse` + public read API); (3i) `celery.py` + trivial task + first Playwright smoke; (3j) output localization — German-first CV *rendering* (filtering already matches cross-language): Django i18n labels + glossary-protected LLM `translate_entries` stored in a lazy-cached, editable `CvEntry.translations` JSONField, target language detected from the posting with an override.
