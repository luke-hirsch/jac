...
| Frontend          | React 19 + Vite + TypeScript. Stack locked 2026-06-02: TanStack Router + Query + Form + Table, Tailwind v4, shadcn/ui. Phase 2a scaffold shipped (foundation + auth guard); Phase 2b (full auth + account pages) in progress. |
...
| [backend/jac/serializers.py](backend/jac/serializers.py) | DRF serializers for all 8 career models (incl. `Skill.years_of_experience` computed field); `ScopeDomainsToUserMixin` extending the project-level base |
...
React 19 + Vite + TS at `frontend/`. Phase 2a (foundation) is committed; Phase 2b (auth + account pages) is in progress per [.claude/plans/phase-2b-setup-guide.md](.claude/plans/phase-2b-setup-guide.md). Backend talks via same-origin Vite proxy (`/api`, `/_allauth`, `/admin`, `/media`, `/static` → `localhost:8000`); router auto-generates `src/routeTree.gen.ts` on file changes.
...
| [frontend/src/routes/_authenticated/account/](frontend/src/routes/_authenticated/account/) | `profile.tsx` (PATCH `/api/spa/profile/`), `email.tsx` (allauth `/account/email`), `security.tsx` (composes the security panels), `danger.tsx` (sessions list + account delete) |
| [frontend/src/routes/auth.tsx](frontend/src/routes/auth.tsx)         | Public `auth` layout — centered card with "back to home" link                                                                                                                 |
| [frontend/src/routes/auth/](frontend/src/routes/auth/)               | `login.tsx`, `signup.tsx`, `verify-email.tsx`, `request-reset.tsx`, `reset-password.$key.tsx`, `mfa-challenge.tsx`                                                            |
...
| [frontend/src/lib/auth.ts](frontend/src/lib/auth.ts)                 | `fetchSession` (treats 401/410 as session payloads, not errors), `useAuth` hook, `useInvalidateSession`, `SessionResponse` / `AuthStatus` types                              |
...
| [frontend/src/lib/reauth.ts](frontend/src/lib/reauth.ts)             | `withReauth(fn)` — runs `fn`; on a `reauthenticate` pending flow, prompts for password, posts to `/auth/reauthenticate`, then retries. Wrap any destructive mutation        |
...
| [frontend/src/components/security/](frontend/src/components/security/) | `change-password.tsx`, `totp-panel.tsx` (QR enroll + recovery codes), `passkey-panel.tsx` (list + add + remove) — all use `withReauth`                                       |
...
- **Phase 2b — auth + account pages (in progress).** Auth routes: `/auth/{login,signup,verify-email,request-reset,reset-password/$key,mfa-challenge}` driving headless allauth. Account routes under `/_authenticated/account/{profile,email,security,danger}` with shared `AccountLayout`. Security panels in `src/components/security/`: `ChangePassword`, `TotpPanel` (QR enroll + recovery codes), `PasskeyPanel`. Helpers: `auth-flow.ts` (`resolveAuthOutcome` discriminated union), `reauth.ts` (`withReauth` wraps destructive mutations and prompts for password on `reauthenticate` pending flow), `webauthn.ts` (base64url codec + creation/assertion encoders with typed `Encoded*Options`), `form.ts` (zod ↔ TanStack Form bridge). Live setup guide at [.claude/plans/phase-2b-setup-guide.md](.claude/plans/phase-2b-setup-guide.md).

**Next, in order** (full detail in the roadmap):

- **Phase 2c/2d — JAC CRUD + LLM connector UI** — full CRUD for all career models + LLM connector config UI.
